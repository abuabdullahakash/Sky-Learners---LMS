            ...
            <>
              ...
              <div className="flex flex-col gap-3">
                {course.sliderImages.map((_: any, idx: number) => (
                  <div key={idx} onClick={() => setCurrentSlide(idx)} className={`w-2 rounded-full transition-all duration-300 cursor-pointer ${idx === currentSlide ? 'bg-gradient-to-b from-blue-400 to-pink-500 h-8 shadow-[0_0_10px_rgba(236,72,153,0.5)]' : 'bg-white/50 h-2 hover:bg-white/80'}`} />
                ))}
              </div>
